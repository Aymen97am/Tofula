...
class TofuulaService:
    ...
    def convert_story_output_to_template_pages(
        ...
        if story_output.outline and hasattr(story_output.outline, 'beats'):
            for beat in story_output.outline.beats:
                ...
                base_prompt = ""
                if story_output.metadata and 'illustration_prompts' in story_output.metadata:
    ...
    def generate_personalized_story_from_template(
        ...
        """
        logger.info(
            f"Personalizing story {template.id} for {request.child_name}"
        )
    ...
    def generate_illustrations_for_pages(
        ...
        image_client = self.pipeline.image_client if hasattr(self.pipeline, 'image_client') else None
        ...
        for page in pages:
            ...
            try:
                ...
                logger.error(f"Failed to generate illustration for page {page_num}: {e}")
    ...
    def _replace_placeholders(
        self, text: str, child_name: str, age: int
    ) -> str:
    ...
    def _personalize_prompt(
        self, base_prompt: str, appearance
    ) -> str:
